...
describe('NodeFileSystemAdapter', () => {
  ...
  it('should write outputs to .context/dotstack/ folder when .context/ is present', async () => {
    const contextDir = path.join(testRoot, '.context');
    await fs.mkdir(contextDir, { recursive: true });

    const { yamlPath, mdPath } = await adapter.writeOutputs(mockRecommendation, testRoot);

    expect(yamlPath).toBe(path.join(testRoot, '.context', 'dotstack', 'stack.yaml'));
    expect(mdPath).toBe(path.join(testRoot, '.context', 'dotstack', 'README.md'));
